...
namespace Space2Ace_Draven
{
    class Program
    {
        ...
        private static void FinishLoading()
        {
            ...
            Menu = MainMenu.AddMenu("Space2Ace_Draven", "Space2Ace_Draven");

            AutoCatch = (Menu.Add("Space2Ace_Draven_auto", new CheckBox("Auto catch axes?", true))).CurrentValue;
            CatchOnlyCloseToMouse = Menu.Add("Space2Ace_Draven_onlyclose", new CheckBox("Catch only axes close to mouse?", true)).CurrentValue;
            MaxDistToMouse = Menu.Add("Space2Ace_Draven_maxdist", new Slider("Max axe distance to mouse", 800, 250, 1250)).CurrentValue;
            OnlyCatchIfSafe = Menu.Add("Space2Ace_Draven_safeaxes", new CheckBox("Only catch axes if safe (anti melee)", false)).CurrentValue;
            MinQLaneclearManaPercent = Menu.Add("Space2Ace_Draven_QLCMana", new Slider("Min Mana Percent for Q Laneclear", 60, 0, 100)).CurrentValue;

            EMenu = Menu.AddSubMenu("Space2Ace_Draven_E", "E Settings: ");
            ECombo = EMenu.Add("Space2Ace_Draven_ECombo", new CheckBox("Use E in Combo", true)).CurrentValue;
            EGC = EMenu.Add("Space2Ace_Draven_EGC", new CheckBox("Use E on Gapcloser", true)).CurrentValue;
            EInterrupt = EMenu.Add("Space2Ace_Draven_EInterrupt", new CheckBox("Use E to Interrupt", true)).CurrentValue;

            RMenu = Menu.AddSubMenu("Space2Ace_Draven_R", "R Settings:");
            RKS = RMenu.Add("Space2Ace_Draven_RKS", new CheckBox("Use R to steal kills", true)).CurrentValue;
            RKSOnlyIfCantAA = RMenu.Add("Space2Ace_Draven_RKSOnlyIfCantAA", new CheckBox("Use R KS only if can't AA", true)).CurrentValue;
            RIfHit = RMenu.Add("Space2Ace_Draven_RIfHit", new Slider("Use R if it will hit X enemies", 2, 1, 5)).CurrentValue;
            R1vs1 = RMenu.Add("Space2Ace_Draven_R1v1", new CheckBox("Always use R in 1v1", true)).CurrentValue;
            WCombo = Menu.Add("Space2Ace_Draven_WCombo", new CheckBox("Use W in Combo", true)).CurrentValue;
            UseItems = Menu.Add("Space2Ace_Draven_Items", new CheckBox("Use Items?", true)).CurrentValue;
        }
