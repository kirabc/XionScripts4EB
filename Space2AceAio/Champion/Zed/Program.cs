    ...
    class Program
    {
        ...
        public static void Game_OnGameLoad()
        {
            try
            {
                ...
                _config = MainMenu.AddMenu("Space2Ace Zed", "Space2Ace Zed");
                ...
                miscMenu.Add("FleeW", new KeyBind("Use W To Flee", false, KeyBind.BindTypes.HoldActive, 'T'));
                foreach (var e in enemy)
                {
                    SpellDataInst rdata = e.Spellbook.GetSpell(SpellSlot.R);
                    if (DangerousDB.DangerousList.Any(spell => spell.Contains(rdata.SData.Name)))
        ...
        private static void OnProcessSpell(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs castedSpell)
        {
            ...
            if (unit.IsEnemy)
            {
                ...
                {
                    if (DangerousDB.DangerousList.Any(spell => spell.Contains(castedSpell.SData.Name)) &&
