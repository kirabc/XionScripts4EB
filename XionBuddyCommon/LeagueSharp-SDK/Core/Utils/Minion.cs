    ...
    public static class Minion
    {
        ...
        private static readonly List<string> SuperMinionList = new List<string>
                                                                   {
                                                                       ...
                                                                       "OdinRedUltraminion","OdinBlueUltraminion_"
                                                                   };
            ...
            int useMecMax = 9)
        {
            ...
            if (minionPositions.Count <= useMecMax)
            {
                ...
                foreach (var subGroup in subGroups)
                {
                    if (subGroup.Count > 0)
                    {
                        ...
                        if (circle.Radius <= width && circle.Center.LSDistanceSquared(startPos) <= range)
                        {
            ...
            else
            {
                ...
                {
                    if (pos.LSDistanceSquared(startPos) <= range)
                    {
                        var count = minionPositions.Count(pos2 => pos.LSDistanceSquared(pos2) <= width * width);
        ...
        public static FarmLocation GetBestLineFarmLocation(List<Vector2> minions, float width, float range)
        {
            ...
            {
                if (pos.LSDistanceSquared(startPos) <= range * range)
                {
